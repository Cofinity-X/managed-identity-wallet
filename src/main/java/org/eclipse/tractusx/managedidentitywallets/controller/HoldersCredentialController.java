...
public class HoldersCredentialController extends BaseController {
    ...
    public ResponseEntity<PageImpl<VerifiableCredential>> getCredentials(
            ...
            @Parameter(name = "issuerIdentifier", description = "Issuer identifier(did of BPN)", examples = {
                    @ExampleObject(name = "bpn", value = "BPNL000000000000", description = "bpn"),
                    @ExampleObject(description = "did", name = "did", value = "did:web:localhost:BPNL000000000000") }) @RequestParam(required = false) String issuerIdentifier,
            ...
            @Min(0) @Max(Integer.MAX_VALUE) @Parameter(description = "Number of records per page") @RequestParam(required = false, defaultValue = Integer.MAX_VALUE
                    + "") int size,
            Principal principal) {
        return ResponseEntity.status(HttpStatus.OK).body(holdersCredentialService.getCredentials(credentialId,
                issuerIdentifier, type, sortColumn, sortTpe, pageNumber, size, getBPNFromToken(principal)));
    }
